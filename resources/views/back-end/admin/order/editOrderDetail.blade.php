...
@section('back-end.contents')
    ...
    <div class="overflow-x-auto flex flex-col">
        <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" method="POST"
            action="{{ route('orderDetail.update') }}">
            @csrf
            @method('POST')
                ...
                <div class="relative w-80">
                    <select name="product" disabled="disabled"
                        class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
                            <option value="{{ $detail->productID }}">{{ $detail->name }}</option>
                    </select>
            ...
            <div class="mb-6">
                ...
                <input required min="1" name="quantity"
                    class="@error('quantity') is-invalid @enderror shadow appearance-none border rounded w-48 py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
